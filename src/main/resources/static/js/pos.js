...
function selectSearchProduct(variantId, name, price) {
    ...
    resultsDiv.innerHTML = `<div class="pos-result-item">
                ...
                <div class="sug-action">
                    ...
                    <button class='pos-btn' onclick='addToCart(${sp.id}, "${sp.tenSanPham.replace(/"/g, '&quot;')}", ${sp.giaBan || 0})'>${i18n.addToCart}</button>
                </div>
            </div>`;
}
...
function searchProducts() {
    ...
    fetch(`/pos/products?keyword=${encodeURIComponent(keyword)}`)
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                resultsDiv.innerHTML = `<em>${document.getElementById("i18n-notFound").innerText}</em>`;
                return;
            }
            const f = new Intl.NumberFormat('vi-VN');
            data.forEach(sp => {
                ...
                const img = sp.image || '/images/no-image.svg';
                const addToCartText = document.getElementById("i18n-addToCart").innerText;
    ...
    <div class="sug-action">
        ...
        <button class='pos-btn' onclick='addToCart(${sp.id}, "${sp.tenSanPham.replace(/"/g, '&quot;')}", ${sp.giaBan || 0})'>
            ${addToCartText}
        </button>
    </div>`;
...
function renderCart() {
    ...
    cart.forEach((item, i) => {
        ...
        <td>${new Intl.NumberFormat('vi-VN').format(thanhTien || 0)}</td>
            <td><button onclick='removeCart(${i})'>
    ${document.getElementById("i18n-delete").innerText}
</button></td>
`;
